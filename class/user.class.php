...
public function getInstaGrowthGenStats($cstring, $username) {
    // SQL query
    $sql = "
        SELECT 
            MIN(start_time) AS first_start_time, 
            MAX(finish_time) AS last_finish_time, 
            SUM(total_unfollowed) AS total_unfollowed, 
            SUM(total_interactions) AS total_interactions, 
            SUM(total_followed) AS total_followed, 
            SUM(total_likes) AS total_likes, 
            SUM(total_watched) AS total_watched 
        FROM 
            instagram_session 
        WHERE 
    ...
    $stmt = mysqli_prepare($cstring, $sql);
    ...
    // Check for errors
    if (!$result) {
        echo "Error: " . mysqli_error($cstring);
        return null;
    }
    ...
    if ($data['first_start_time']) {
        $firstStartTime = DateTime::createFromFormat('Y-m-d H:i:s', $data['first_start_time']);
        $data['first_start_time'] = $firstStartTime ? $firstStartTime->format('d/m/Y') : null;
    }

    if ($data['last_finish_time']) {
        $lastFinishTime = DateTime::createFromFormat('Y-m-d H:i:s', $data['last_finish_time']);
        $data['last_finish_time'] = $lastFinishTime ? $lastFinishTime->format('Y/m/d') : null;
    }
...
// Function to retrieve Instagram interaction data without hashtags as target
public function getInstaTopInteractions($cstring, $username) {
    // Updated SQL query to exclude targets starting with '#'
    $sql = "
        SELECT 
            SUM(liked) AS liked, 
            SUM(pm_sent) AS pm_sent, 
            SUM(watched) AS watched, 
            SUM(unfollowed) AS unfollowed, 
            SUM(followed) AS followed_total, 
            target 
        FROM 
            instagram_interact 
        WHERE 
            instagram_username_interact = ? 
            AND target NOT LIKE '#%'  -- Exclude hashtags
        GROUP BY 
            target 
        ORDER BY 
            followed_total DESC 
...
// Function to retrieve Instagram interaction data without hashtags as target
public function getInstaFlopInteractions($cstring, $username) {
    // Updated SQL query to exclude targets starting with '#'
    $sql = "
        SELECT 
            SUM(liked) AS liked, 
        ...
        WHERE 
            instagram_username_interact = ? 
            AND target NOT LIKE '#%'  -- Exclude hashtags
			AND (is_unlist = 0 OR is_unlist IS NULL)  -- Explicitly exclude is_unlist = 1
        GROUP BY 
            target 
        ORDER BY 
            followed_total ASC
...
//get targets list
public function getTargets($cstring, $username ) {
    $query = "SELECT DISTINCT target FROM instagram_interact WHERE is_follow_back_test=1 and instagram_username_interact=?";
	
    
	$stmt = mysqli_prepare($cstring, $query);
	if ($stmt === false) {
		// Handle error in preparing the statement
		die('Error preparing statement: ' . $cstring->error);
	}
	...
	mysqli_stmt_execute($stmt);
	$result = mysqli_stmt_get_result($stmt);
	// Initialize an array to hold all result values
    $targets = [];
    
    // Fetch each row as an associative array and add it to $targets
    while ($row = mysqli_fetch_assoc($result)) {
...
function getTotalTestsCount($mysqli, $target, $username) {
    // Prepare the SQL query
    $query = "SELECT COUNT(*) FROM instagram_interact WHERE is_follow_back_test=1 AND target=? AND instagram_username_interact=?";
    ...
    // Bind the parameters to the query ('ss' stands for string for both parameters)
    mysqli_stmt_bind_param($stmt, 'ss', $target, $username);
    
    // Execute the query
    mysqli_stmt_execute($stmt);
    
    // Bind the result variable to hold the count
...
function calculateFollowBackPercentages($followBackCount, $totalTestsCount) {
    // Calculate the follow-back percentage
    if ($totalTestsCount > 0) {
        $percentage = ($followBackCount / $totalTestsCount) * 100;
    } else {
...
function getFollowBackCount($mysqli, $target, $username) {
    // Prepare the SQL query
    $query = "SELECT COUNT(*) FROM instagram_interact WHERE is_following_back=1 AND target=? AND instagram_username_interact=?";
    ...
    // Bind the parameters to the query ('s' stands for string)
    mysqli_stmt_bind_param($stmt, 'ss', $target, $username);
    ...
    // Get the result
    mysqli_stmt_bind_result($stmt, $count); // Bind the result variable
    mysqli_stmt_fetch($stmt); // Fetch the result into the bound variable
...
// Function to retrieve Instagram interaction data without hashtags as target
public function getInstaTopHashtagsInteractions($cstring, $username) {
    // Updated SQL query to exclude targets starting with '#'
    $sql = "
        SELECT 
            SUM(liked) AS liked, 
        ...
        WHERE 
            instagram_username_interact = ? 
            AND target LIKE '#%'  -- Include only hashtags
        GROUP BY 
            target 
        ORDER BY 
            followed_total DESC 
...
/**
 * Retrieves the first and last session entries for a given instagram_username_interact using MySQLi.
 *
 * @param mysqli $mysqli - The MySQLi connection object.
 * @param string $instagram_username_interact - The username to filter the sessions by.
 * @return array - An associative array with 'first' and 'last' keys, containing the respective session records or empty arrays if not found.
 */
public function getFirstAndLastInstagramSession($mysqli, string $instagram_username_interact): array {
    try {
        // SQL query to fetch the first session entry
        $firstQuery = "
            SELECT 
                profile_posts, 
                profile_followers, 
        ...
        // SQL query to fetch the last session entry
        $lastQuery = "
            SELECT 
                profile_posts, 
                profile_followers, 
        ...
        $stmtFirst = mysqli_prepare($mysqli, $firstQuery);
        if (!$stmtFirst) {
            throw new Exception("Failed to prepare first query: " . mysqli_error($mysqli));
        }
        
        // Bind the parameter and execute the first query
        mysqli_stmt_bind_param($stmtFirst, 's', $instagram_username_interact);
        mysqli_stmt_execute($stmtFirst);
        $firstResult = mysqli_stmt_get_result($stmtFirst);
        $firstEntry = mysqli_fetch_assoc($firstResult);

        // Prepare the last query
        $stmtLast = mysqli_prepare($mysqli, $lastQuery);
        if (!$stmtLast) {
            throw new Exception("Failed to prepare last query: " . mysqli_error($mysqli));
        }

        // Bind the parameter and execute the last query
        mysqli_stmt_bind_param($stmtLast, 's', $instagram_username_interact);
        mysqli_stmt_execute($stmtLast);
        $lastResult = mysqli_stmt_get_result($stmtLast);
        $lastEntry = mysqli_fetch_assoc($lastResult);

        // Close the prepared statements
        mysqli_stmt_close($stmtFirst);
        mysqli_stmt_close($stmtLast);

        // Return the first and last entry in a structured array
        return [
            'first' => $firstEntry ? $firstEntry : [],  // First session entry or empty array
            'last' => $lastEntry ? $lastEntry : []      // Last session entry or empty array
        ];
