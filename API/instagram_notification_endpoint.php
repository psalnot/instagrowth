...
class NotificationsEndpoint {
     ...
     */
    private function notificationExists(string $username, string $username_interact, ?string $target = null, ?string $job_name = null): bool {
        ...
        // Check if both target and job_name are provided and not null
        $useDetailedCheck = !empty($target) && !empty($job_name);
        ...
        $this->debug('Notification check result', [
            ...
            'count' => $result['nb'],
            'used_detailed_check' => $useDetailedCheck
        ]);
        error_log("Instagram Notification API - Notification check result: " . print_r($result, true));
    ...
    private function storeNotification(array $data): int {
        ...
        error_log("Instagram Notification API - Full Data Array: " . print_r($data, true));
        ...
        try {
            ...
            // Determine values based on presence of date fields
            $isChecked = isset($data['target']) && $data['target'] !== null && $data['target'] !== '';
            $lastInteraction = (!empty($data['last_interaction'])) ? $data['last_interaction'] : null;
            $followedDate = (!empty($data['followed_date_notification'])) ? $data['followed_date_notification'] : null;
            $likedDate = (!empty($data['like_date_notification'])) ? $data['like_date_notification'] : null;
            $isFollowed = !empty($data['followed_date_notification']) ? 1 : 0;
            $isLiked = !empty($data['like_date_notification']) ? 1 : 0;
            
            // Determine if user has followed back based on followed_date and interaction flags
            $hasInteracted = isset($data['followed']) && $data['followed'] || 
                            isset($data['likes']) && $data['likes'] || 
                            isset($data['interacted']) && $data['interacted'];
            
            // Check if followed_date_notification exists and is a valid date
            $hasValidFollowedDate = isset($data['followed_date_notification']) && 
                                   $data['followed_date_notification'] !== null && 
                                   DateTime::createFromFormat('Y-m-d', $data['followed_date_notification']) !== false;
            
            $isFollowBack = $hasValidFollowedDate && $hasInteracted ? 1 : 0;
            ...
            if ($exists) {
                ...
                $stmt->bindValue(':last_interaction', $lastInteraction, PDO::PARAM_STR);
                ...
                $stmt->bindValue(':followed_date', $followedDate, PDO::PARAM_STR);
                $stmt->bindValue(':liked_date', $likedDate, PDO::PARAM_STR);
                $stmt->bindValue(':is_followed', $isFollowed, PDO::PARAM_INT);
                $stmt->bindValue(':is_liked', $isLiked, PDO::PARAM_INT);
                $stmt->bindValue(':is_follow_back', $isFollowBack, PDO::PARAM_INT);
